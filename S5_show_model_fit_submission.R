...
for(j in 1:nm){
  ...
  cMFCV = MFCV[[j]]
  if(!is.null(cMF$RMSE)){
    plot(cMF$RMSE,cMFCV$RMSE,xlim=c(0,1),ylim=c(0,1),
         xlab = "explanatory power",
         ylab = "predictive power",
         main=paste0(modelnames[[j]],", thin = ",as.character(thin),", samples = ",as.character(samples),": RMSE"))
    abline(0,1)
    abline(v=0)
    abline(h=0)
  }
  if(!is.null(cMF$R2)){
    plot(cMF$R2,cMFCV$R2,xlim=c(-0.3,1),ylim=c(-0.3,1),
         xlab = "explanatory power",
         ylab = "predictive power",
  ...
  if(!is.null(cMF$TjurR2)){
    plot(cMF$TjurR2,cMFCV$TjurR2,xlim=c(-0.3,1),ylim=c(-0.3,1),
         xlab = "explanatory power",
         ylab = "predictive power",
